...
export function getUser(db: any, username: string) {
    const findUser = new PQ({text: 'SELECT * FROM public."User" WHERE login = $1', values: [username]});
...
export function getUsers(db: any) {
  const findUsers = new PQ({text: 'SELECT * FROM public."User"'});
...
export function getUsersByLocation(db: any, location: string) {
  const findUsers = new PQ({text: 'SELECT * FROM public."User" WHERE LOWER(location) = LOWER($1)', values: [location]});
...
export function getUsersByLanguage(db: any, language: string) {
  const findUsers = new PQ({text: 'SELECT * FROM public."User" where $1=ANY(languages)', values: [language.toLowerCase()]});
...
export function getUsersBy(db: any, location: string, language: string) {
    ...
    const findUsers = new PQ({text: `SELECT * FROM public."User" WHERE ${whereClause}`, values: values});
